...
public class UserKey {
  ...
  */
  private static boolean isConsume(KeyEvent evt, int mod) {
    if (OperatingSystem.isMacOS()) {
      if (!(evt.isMetaDown() && !evt.isAltGraphDown())) {
        return false;
      }
      if (evt.isAltDown() && (mod & ALLOW_CTRL) == 0) {
        return false;
      }
    } else {
      if (!(evt.isAltDown() && !evt.isAltGraphDown() && !evt.isMetaDown())) {
        return false;
      }
      if (evt.isControlDown() && (mod & ALLOW_CTRL) == 0) {
        return false;
      }
    }
    
    if (evt.isShiftDown() && (mod & ALLOW_SHIFT) == 0) {
      return false;
    }
  ...
  */
  ...
  int MyMASK = (OperatingSystem.isMacOS() ? KeyEvent.META_MASK : KeyEvent.ALT_MASK);
  
  switch (evt.getKeyCode()) {
  case KeyEvent.VK_I:
    if (blnUP_DOWN && isConsume(evt, mod_up)) {
      ...
      Toolkit.getDefaultToolkit().getSystemEventQueue().postEvent(
        new KeyEvent((Component)evt.getSource(), evt.getID(),
          evt.getWhen(), evt.getModifiers() & ~MyMASK,
    ...
    break;
  case KeyEvent.VK_K:
    if (blnUP_DOWN && isConsume(evt, mod_down)) {
      // if (evt.isAltDown() && !evt.isAltGraphDown()
      // && !evt.isMetaDown() && !evt.isShiftDown()) {
      Toolkit.getDefaultToolkit().getSystemEventQueue().postEvent(
        new KeyEvent((Component)evt.getSource(), evt.getID(),
    ...
    break;
  case KeyEvent.VK_J:
    if (blnLEFT_REIGHT && isConsume(evt, mod_left)) {
      // if (evt.isAltDown() && !evt.isAltGraphDown()
    ...
    break;
  case KeyEvent.VK_L:
    if (blnLEFT_REIGHT && isConsume(evt, mod_right)) {
      // if (evt.isAltDown() && !evt.isAltGraphDown()
      // && !evt.isControlDown() && !evt.isMetaDown()
      // && !evt.isShiftDown()) {
      Toolkit.getDefaultToolkit().getSystemEventQueue().postEvent(
  ...
  case KeyEvent.VK_SEMICOLON:
    ...
    break;
  case KeyEvent.VK_0:
    if (blnEdit && isConsume(evt, ALLOW_CTRL)) {
      Toolkit.getDefaultToolkit()
      .getSystemEventQueue().postEvent(
        new KeyEvent((Component)evt.getSource(), evt.getID(),
          evt.getWhen(), evt.getModifiers() & ~MyMASK,
  ...
  case KeyEvent.VK_COMMA:
    if (blnHistory && isConsume(evt, ALLOW_CTRL | ALLOW_SHIFT)) {
      Toolkit.getDefaultToolkit()
      .getSystemEventQueue().postEvent(
        new KeyEvent((Component)evt.getSource(), evt.getID(),
          evt.getWhen(), evt.getModifiers() & ~MyMASK | KeyEvent.ALT_MASK,
          KeyEvent.VK_DOWN, KeyEvent.CHAR_UNDEFINED)
        );
      evt.consume();
    }
